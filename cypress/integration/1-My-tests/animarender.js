describe('[+] Проверка оплаты на сайте https://www.animarender.com/ru', function () {
   it('Автотест на оплату в личном кабинете', function () {
        cy.visit('https://account.animarender.com/login');
        cy.get('#login').type('email')
        cy.get('#password').type('password')
        cy.get('.mx-auto').click()
        cy.get('.balance').click()
        cy.get('.inputs__money-block > .v-input-field > .v-input-box > .v-input-wrapper > .input').type('{selectall}{backspace}5')
        cy.get('.sign').click()
    })    
})
